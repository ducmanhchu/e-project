...
} from "@server/services/writingService";

const VOCAB_POPULATE = {
	path: "vocabularyRefs.id",
	select: "word partOfSpeech ipa definitions",
};

/**
 * Map order → referenceAnswer để normalize từ vựng theo ngữ cảnh câu.
 * @param {Array<{ order: number, referenceAnswer: string }>} sentences
 * @returns {Map<number, string>}
 */
function buildSentenceRefMap(sentences) {
	const map = new Map();
	for (const s of sentences || []) {
		if (s.order != null && s.referenceAnswer) {
			map.set(s.order, s.referenceAnswer);
		}
	}
	return map;
}
...
/**
 * Map vocabularyRefs đã populate sang response admin.
 * @param {import("mongoose").LeanDocument} lesson
 */
function mapPopulatedVocabulary(lesson) {
	return (lesson.vocabularyRefs || [])
		.filter((ref) => ref.id)
		.map((ref) => ({
...
export async function updateLesson(lessonId, body) {
	...
	if (body.vocabulary !== undefined) {
		const totalSentences = updates.totalSentences ?? lesson.totalSentences;
		validateVocabularyEntries(body.vocabulary, totalSentences);

		const sentenceRefMap = buildSentenceRefMap(
			updates.sentences ?? lesson.sentences,
		);

		if (body.vocabulary.length === 0) {
			updates.vocabularyRefs = [];
		} else {
			updates.vocabularyRefs = await buildVocabularyRefs(
				body.vocabulary,
				sentenceRefMap,
			);
		}
	...
	return {
		...
		vocabulary: mapPopulatedVocabulary(updated),
...
/**
 * DELETE /admin/writing/reverse-translation?ids=a,b,c — Bulk delete + cascade
 */
export async function bulkDeleteLessons(ids) {
	const docs = await ReverseTranslation.find({ _id: { $in: ids } })
		.select("_id")
		.lean();
	...
	await Promise.all([
		ReverseTranslation.deleteMany({ _id: { $in: docIds } }),
		attemptIds.length
...
export async function adminGetLesson(lessonId) {
	...
	return {
		...
		vocabulary: mapPopulatedVocabulary(lesson),
		createdAt: lesson.createdAt,
