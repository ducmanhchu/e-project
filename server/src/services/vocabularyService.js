...
export async function addWord(userId, payload) {
	const { vocabularyId, word, partOfSpeech } = payload;
...
export async function createVocabulary(payload) {
	...
	if (mode !== "manual" && mode !== "enrich") {
		throw ApiError.badRequest(
			'mode is required and must be "manual" or "enrich"',
		);
	}
	...
	if (existing)
		throw ApiError.conflict(`Vocabulary "${normalized}" already exists`);
	...
	if (mode === "manual") {
		...
		if (!Array.isArray(definitions) || definitions.length === 0) {
			throw ApiError.badRequest(
				"definitions must be a non-empty array in manual mode",
			);
		}
