...
export async function createDialogue(req, res, next) {
	try {
		...
		const data = await slangHangService.createDialogue(req.body);
...
/**
 * PUT /api/admin/slang-hang/dialogues/:id — [ADMIN] Cập nhật một phần
 */
export async function updateDialogue(req, res, next) {
	try {
		validateObjectId(req.params.id, "id");
		const data = await slangHangService.updateDialogue(req.params.id, req.body);
...
/**
 * GET /api/admin/slang-hang/dialogues — [ADMIN] Danh sách (không có status)
 */
...
/**
 * GET /api/admin/slang-hang/dialogues/:id — [ADMIN] Chi tiết (không có attempt)
 */
export async function adminGetDialogue(req, res, next) {
	try {
		validateObjectId(req.params.id, "id");
		const data = await slangHangService.adminGetDialogue(req.params.id);
		res.json({ success: true, data });
	} catch (e) {
		next(e);
	}
