...
/**
 * Build a Mongo query fragment that filters Slang Hang dialogues by user attempt status.
 * Same semantics as {@link buildStatusFilter}, but uses DialogueAttempt + dialogueId.
 */
export async function buildDialogueStatusFilter({ userId, statuses }) {
	if (!Array.isArray(statuses) || statuses.length === 0) return null;
	...
	const attempts = await DialogueAttempt.find({ userId })
 ...
 */
export async function submitAndUpdateProgress(
	attempt,
	{
		sentenceOrder,
		userAnswer,
	...
	const completedCount = attempt.sentenceProgress.filter(
		(p) => p.isCompleted,
	).length;
	attempt.completedSentences = completedCount;

	const scoredProgress = attempt.sentenceProgress.filter(
		(p) => p.bestScore > 0,
	);
	attempt.bestScore =
		scoredProgress.length > 0
			? Math.round(
					scoredProgress.reduce((sum, p) => sum + p.bestScore, 0) /
