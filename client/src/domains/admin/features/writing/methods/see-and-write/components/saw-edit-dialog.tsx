...
export function SAWEditDialog({
	...
	useEffect(() => {
		...
		if (exercise) {
			setOriginalImage(exercise.image);
			form.reset(exerciseToFormValues(exercise));
			resetImageFile();
		}
	}, [open, exercise, form, resetImageFile, resetLocalState]);

	const updateMutation = useMutation({
		mutationFn: async (values: SAWEditFormValues) => {
			if (!exerciseId) throw new Error("MISSING_ID");
		...
		onSuccess: async () => {
			...
			if (exerciseId) {
				await queryClient.invalidateQueries({
					queryKey: ["admin", "see-and-write", "detail", exerciseId],
				});
			}
