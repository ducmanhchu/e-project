...
export function useParaphrase(exerciseId: string) {
	...
	// Order của câu chưa hoàn thành đầu tiên — null khi toàn bộ đã xong
	const firstIncompleteOrder = useMemo(() => {
		const target = sentences.find((s) => !s.isCompleted);
		return target?.order ?? null;
	}, [sentences]);

	// Order lớn nhất user được phép truy cập: câu đã hoàn thành + câu dở hiện tại.
	...
	const isAllCompleted =
		firstIncompleteOrder === null && sentences.length > 0;
	...
	const isInputDisabled =
		isSubmitting || (isViewingCompleted && !isRetrying);
